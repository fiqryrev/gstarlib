gstar.fit.gls = function(x,arord,dif,W,insample,intercept=FALSE,lbd){
  #================================================================
  #          Fit GSTAR Model with Generalized Least Square        #
  #================================================================
  d_dtx    = dim(x)[1]
  d_lokasi = dim(x)[2]
  #================================================================
  #                        GSTAR OLS Fit                          #
  #================================================================   
  fitols            = gstar.fit.ols(x=x,dif=dif,arord=arord,W=W,insample=insample,intercept=intercept,lbd=lbd)
  ols_vtotal	      = as.matrix(fitols$Emat)
  ed_ols            = dim(ols_vtotal)
  edt_ols           = ed_ols[1]*ed_ols[2]
  ols_v             = matrix(0,ed_ols[1],ed_ols[2])
  ols_vk            = matrix(0,ed_ols[1],ed_ols[2])
  ols_vkx           = matrix(0,ed_ols[1],ed_ols[2])
  for (i in 1:d_lokasi){
    ols_v[,i]      = as.vector(ols_vtotal[,i])
    ols_vk[,i]     = ols_v[,i]^2 
    ols_vkx[,i]    = ols_vk[,i]
  }
  #================================================================
  #		              Calculating Matrix Sigma GLS		  	          #        
  #================================================================    
  Matcov   = cov(ols_v)
  MatId    = diag(1,dim(ols_v))
  matH     = Matcov%x%MatId
  sigma    = solve(matH)
  #================================================================
  #                      Generating Values                        #
  ...
  for (z in 1:dd1){
    ZT = c(ZT,zt[z,])
    if(z==dd1) ZT = c(as.matrix(unlist(ZT)))   #Mencari Y
  }
  ztp    = vt = c()
  ZVList = list()
  orde.ar= -1L

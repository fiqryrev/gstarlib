gstar.fit.mle = function(x,arord,dif,W,insample,intercept=FALSE,lbd){
  #================================================================
  #       Fit GSTAR Model with Maximum Likelihood Function        #
  #================================================================
  MLEProcess = TRUE
  d_dtx      = dim(x)[1]
  d_lokasi   = dim(x)[2]
  ...
  for(i in order.min:(order.max+1)){
    ...
    if(intercept==FALSE&orde.ar==0){
      ...
      loglikhit=.5* (sum(log(wll)) - d1 * (log(2 * pi) + 1 - log(d1) +
                                             log(sum(E[[i]]^2))))
  ...
  if(MLEProcess){
    ...
    #MLE Function
    mleGSTAR = optim(thetaGSTAR,               
                     ols.lf,                            
                     control=list(trace=1, fnscale=-1), 
                     K=MLE_K, y=ZT, X=X                    
    )
    #MLE Output
    A    = t(as.matrix(mleGSTAR$par[-length(mleGSTAR$par)]))
    sdE  = as.matrix(mleGSTAR$par[length(mleGSTAR$par)])
    #MLE Reprocess
    XXI = solve(crossprod(X))
    Yhat= A%*%t(X)
    E   = (ZT-Yhat)
    VarE= as.matrix(tcrossprod(E)/length(ZT))
    VarA= XXI%x%VarE
    seA = sqrt(diag(VarA))
    xaic  = d1*log(det(VarE))+2*(ncol(X)*orde.ar)
    xaicc = log(det(VarE))+((d1+1)/(d1-ncol(X)*orde.ar-1))
    xbic  = log(d1)*ncol(X)*orde.ar - 2*log(det(VarE))
    xsic  = log(det(VarE))+(ncol(X)*orde.ar*log(d1))/d1
